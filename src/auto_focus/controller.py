...
class AutoFocusController:
    ...
    def _update_cursor(self, frame: np.ndarray, box: BoundingBox) -> None:
        ...
        self._smoothed_offset = (
            (1 - self.smoothing_factor) * self._smoothed_offset
            + self.smoothing_factor * normalized_offset
        )
    ...
    def _apply_3d_cursor_movement(self, move_vector: np.ndarray) -> None:
        ...
        dx, dy = float(move_vector[0]), float(move_vector[1])

        if hasattr(self.cursor_controller, "moveRel"):
            self.cursor_controller.moveRel(dx, dy)

        # When an application consumes relative input (e.g. a 3D game) the OS cursor is
        # typically locked to the screen centre. Best effort re-centre the pointer so the
